...
- (instancetype)init {
    ...
    if (self = [super init]) {
        ...
        SLog(@"[YCDownloadSession init] savePath: %@", [YCDownloadSession getArchiverPath]);
        ...
        [dictM enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, NSURLSessionDownloadTask *obj, BOOL * _Nonnull stop) {
            ...
            if (!task) {
                ...
                SLog(@"[Error] not found task for url: %@", [YCDownloadTask getURLFromTask:obj]);
                [obj cancel];
            } else {
        ...
        //            SLog(@"app start default pause all bg runing task! task count: %zd", dictM.count);
...
- (void)prepareRecreateSession {
    ...
    __block BOOL isCreateSession = YES;
    ...
    [self.downloadTasks enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
        ...
        if (task.downloadTask.state == NSURLSessionTaskStateRunning) {
            ...
            //只有等待中/下载中，重启后恢复自动下载
            if (task.downloadStatus == YCDownloadStatusDownloading || task.downloadStatus == YCDownloadStatusWaiting) {
    ...
    /**
     *取消会话，取消后会调用：- (void)URLSession:(NSURLSession *)session didBecomeInvalidWithError:(nullable NSError *)error方法
     */
    [_session invalidateAndCancel];
    
    self.isNeedCreateSession = isCreateSession;
}
...
- (void)resumeAllDownloadTask {
    ...
    SLog(@"[YCDownloadSession resumeAllDownloadTask] no implement!");
}
...
- (void)pauseDownloadTask:(YCDownloadTask *)task {
    ...
    if (!task.isSupportRange) {
        ...
        SLog(@"Error: resource not support resume, because reponse headers not have the filed of 'Accept-Ranges' and 'ETag' !");
    }
}
...
- (void)timerRun {
    ...
    SLog(@"background time remain: %f", [UIApplication sharedApplication].backgroundTimeRemaining);
    
    //TODO: optimeze the logic for background session
    if ([UIApplication sharedApplication].backgroundTimeRemaining < 15 && !_bgRCSBlock) {
        
        SLog(@"background time will up, need to call completed hander!");
...
didFinishDownloadingToURL:(NSURL *)location {
    ...
    if (!task) {
        ...
        SLog(@"[Download Finished] item nil error! url: %@", downloadUrl);
        return;
    }
    ...
    if (!isCompltedFile) {
        ...
        SLog(@"[Download Finished] Error: file size error!");
        return;
    }
...
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didCompleteWithError:(NSError *)error {
    ...
    if (error) {
        ...
        if (resumeData) {
            ...
            SLog(@"[didCompleteWithError] : %@",error);
