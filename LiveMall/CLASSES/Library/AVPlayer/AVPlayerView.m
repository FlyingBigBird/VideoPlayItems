...
-(void)addProgressObserver{
    ...
    _timeObserver = [_player addPeriodicTimeObserverForInterval:CMTimeMake(1, 1) queue:dispatch_get_main_queue() usingBlock:^(CMTime time) {
        ...
        NSLog(@"PlayerStatus:%ld",(long)weakSelf.player.status);

        NSLog(@"PlayItemStatus:%ld",(long)weakSelf.playerItem.status);
...
// 保存至本地相册
-(void)saveVideoWithUrl:(NSString *)url {
    //播放路径
    ...
    //路径作为视频缓存key
    NSString *cacheFileKey = sourceURL.absoluteString;
    
    //查找本地视频缓存数据
    _queryCacheOperation = [[WebCacheHelpler sharedWebCache] queryURLFromDiskMemory:cacheFileKey cacheQueryCompletedBlock:^(id data, BOOL hasCache) {
        
        dispatch_async(dispatch_get_main_queue(), ^{
            //hasCache是否有缓存，data为本地缓存路径
            if(!hasCache) {
                //当前路径无缓存，则将视频的网络路径的scheme改为其他自定义的scheme类型，http、https这类预留的scheme类型不能使AVAssetResourceLoaderDelegate中的方法回调
                [UIAlertView showCustomEnsureAlert:@"视频正在缓冲,请稍后重试"];
            }else {

                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                    
                    if (UIVideoAtPathIsCompatibleWithSavedPhotosAlbum(FORMAT(@"%@",data))) {
                        
                        UISaveVideoAtPathToSavedPhotosAlbum(FORMAT(@"%@",data), self, @selector(video:didFinishSavingWithError:contextInfo:), nil);
                    }
                });
            }
        });
...
- (void)video:(NSString *)videoPath didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInf{
    if (error) {
        ...
        [UIAlertView showCustomEnsureAlert:@"保存失败"];
    }else{
        [UIAlertView showCustomEnsureAlert:@"保存成功"];
    }
}
